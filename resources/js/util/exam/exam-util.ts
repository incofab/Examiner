...
class TabManager {
  ...
  setCurrentQuestion(currentQuestionIndex: number) {
    ...
    this.tabs[this.currentTabIndex] = {
      ...
      exam_item_id: tab.exam_item_id,
    };
    this.reRender();
  }
  getCurrentQuestion() {
    return this.getCurrentCourseSessionQuestions()?.[
  ...
  getCurrentCourseSessionQuestions() {
    const examCourseSession = this.exam.exam_items![this.currentTabIndex];
    const questions = examCourseSession.course_session!.questions!;
    return questions;
  }
}
